...
"""
# function filteropt_rt_optimization_blnoise(filter_type::Symbol, wvfs::ArrayOfRDWaveforms, τ_pz::Quantity{T}, ft::Quantity{T}, 
#     grid_rt::StepRangeLen{Quantity{<:Float64}}, bl_window::ClosedInterval{<:Unitful.Time{T}}, flt_length_cusp::Quantity{T}; 
#     τ_cusp::Quantity{<:AbstractFloat} = 10000000.0u"µs", τ_zac::Quantity{<:AbstractFloat} = 10000000.0u"µs" ) where T<:Real
...
#     wvfs_timestep = step(wvfs[1].time)
...
#     # rt_opt_rough = grid_rt[findfirst(noise .== minimum(noise))]
...
# function filteropt_rt_optimization_blnoise(filter_type::Symbol, wvfs::ArrayOfRDWaveforms, τ_pz::Quantity{T}, dsp_config::DSPConfig; kwargs...) where T<:Real
...
#     filteropt_rt_optimization_blnoise(filter_type, wvfs, τ_pz, ft, grid_rt, bl_window, flt_length_cusp; kwargs...)
# end 

# function filteropt_rt_optimization_blnoise(filter_type::Symbol, wvfs::ArrayOfRDWaveforms, τ_pz::Quantity{T},
#     ft::Quantity{T}, grid_rt::StepRangeLen{Quantity{<:Float64}}, bl_window::ClosedInterval{<:Unitful.Time{T}}; kwargs...) where T<:Real
#     if filter_type !== :cusp
#         filteropt_rt_optimization_blnoise(filter_type, wvfs, τ_pz, ft, grid_rt, bl_window, 0.0u"µs"; kwargs...)
#     else 
...
# function filteropt_trap_rt_blnoise(wvfs::ArrayOfRDWaveforms, dsp_config::DSPConfig, τ_pz::Quantity{T}; ft = 0.0u"µs")
#     # gather config parameters 
#     if ft == 0.0u"µs"
#         (_, ft) = get_fltpars(PropDict(), :trap, dsp_config)
#     end
#     grid_rt   = getproperty(dsp_config, Symbol("e_grid_rt_trap"))
#     bl_window = dsp_config.bl_window
#     wvfs_timestep = step(wvfs[1].time)

#     # waveforms: shift baseline
#     bl_stats = signalstats.(wvfs, leftendpoint(bl_window), rightendpoint(bl_window))
#     wvfs = shift_waveform.(wvfs, -bl_stats.mean)
...
# function filteropt_cusp_rt_blnoise(wvfs::ArrayOfRDWaveforms, dsp_config::DSPConfig, τ_pz::Quantity{T}; ft = 0.0u"µs", τ_cusp::Quantity{<:AbstractFloat} = 10000000.0u"µs")
#     # gather config parameters 
#     if ft == 0.0u"µs"
#         (_, ft) = get_fltpars(PropDict(), :trap, dsp_config)
#     end
#     grid_rt   = getproperty(dsp_config, Symbol("e_grid_rt_cusp"))
#     bl_window = dsp_config.bl_window
...
#         bin_start = bl_start + binscut
#         bin_stop = bl_stop - binscut
#         wvfs_flt = CUSPChargeFilter(rt, ft, τ_cusp, flt_length_cusp, cusp_scale).(wvfs)   # filtered waveforms  
#         bl_trap = filter.(isfinite, map(x-> x.signal[bin_start:1:bin_stop], wvfs_flt)) # baselines - cut bins in beginning and end 
#         noise[i] = rms(vcat(bl_trap...))
#     end
...
# function filteropt_zac_rt_blnoise(wvfs::ArrayOfRDWaveforms, dsp_config::DSPConfig, τ_pz::Quantity{T}; ft = 0.0u"µs", τ_zac::Quantity{<:AbstractFloat} = 10000000.0u"µs")
#     # gather config parameters 
#     if ft == 0.0u"µs"
#         (_, ft) = get_fltpars(PropDict(), :trap, dsp_config)
#     end
#     grid_rt   = getproperty(dsp_config, Symbol("e_grid_rt_zac"))
#     bl_window = dsp_config.bl_window
